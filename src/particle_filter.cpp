				   ...
				   const std::vector<LandmarkObs> &observations, const Map &map_landmarks)
{
	...
	for (unsigned int i = 0; i< num_particles ; i++){
		double px = num_particles[i].x;
		double py = num_particles[i].y;
		double c= cos (num_particles[i].theta);
		double s = sin (num_particles[i].theta);
		for (unsigned int j = 0; j< observations.size(); j++){
			...
			unsigned int k = 0;
			while ( k < map_landmarks.landmark_list.size() $$ obs.id != map_landmarks.landmark_list[k].id_i ){
				k++;
			}
			if (k == landmark_list.size()){
				cout << " ERROR - No map landmark matches the observation"<< endl; 
			}
